...
[ -d "$VAULT_PATH" ] || { echo "vault yok: $VAULT_PATH" >&2; exit 1; }
[ -d "$HERMES_HOME" ] || { echo "hermes yok: $HERMES_HOME" >&2; exit 1; }
...
if grep -qF "$SKILLS_DIR" "$CONFIG" 2>/dev/null; then
  echo "external_dirs: zaten kayitli, atlandi"
else
  cp "$CONFIG" "$CONFIG.bak.$(date +%Y%m%d_%H%M%S)"
  if ! grep -qE "^[[:space:]]+external_dirs:[[:space:]]*\[\][[:space:]]*$" "$CONFIG"; then
    echo "config.yaml icinde 'external_dirs: []' bulunamadi. Elle ekle: external_dirs: ['$SKILLS_DIR']" >&2
    exit 1
  fi
  ...
  mv "$tmp" "$CONFIG"
  echo "external_dirs: eklendi -> $SKILLS_DIR"
fi
...
if grep -qE "^[[:space:]]*OBSIDIAN_VAULT_PATH=" "$ENV_FILE"; then
  ...
  mv "$tmp" "$ENV_FILE"
  echo "OBSIDIAN_VAULT_PATH guncellendi -> $VAULT_PATH"
else
  printf 'OBSIDIAN_VAULT_PATH=%s\n' "$VAULT_PATH" >> "$ENV_FILE"
  echo "OBSIDIAN_VAULT_PATH eklendi -> $VAULT_PATH"
fi

echo
echo "Bitti. Hermes'i yeniden baslat, sonra 'gecen oturumda ne yaptik' diye sor."
